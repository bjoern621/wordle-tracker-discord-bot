 ...
 * message arriving late is ignored, except that one carrying a grid still fills
 * the grid onto a row that has none. Hard mode is only changed by the manual-text
 * sources (others do not report it). See planResultWrite for the full decision.
 */
export async function recordResult(r: ResultRecord): Promise<RecordStatus> {
  ...
  try {
    ...
    if (plan.kind === 'enrich-grid') {
      await client.query(
        `UPDATE results SET grid = $4, updated_at = now()
          WHERE guild_id = $1 AND user_id = $2 AND puzzle_number = $3`,
        [r.guildId, r.userId, r.puzzleNumber, plan.grid],
      );
      await client.query('COMMIT');
      return plan.status;
    }
    ...
    await client.query(
      `INSERT INTO results
       ...
       ON CONFLICT (guild_id, user_id, puzzle_number) DO UPDATE SET
         ...
         updated_at  = now()`,
      [r.guildId, r.userId, r.puzzleNumber, r.puzzleDate, r.guesses, r.solved, plan.grid, plan.hardMode, r.source, r.messageTs, r.username, r.messageId],
    );
